<html>
	<head>
        <title>apoteksenja</title>
        <meta charset="utf-8"></meta>
        <meta name="description" content="apotek"></meta>
        <meta name="author" content="kelompokapotek"></meta>
        <meta name='date'content='25 Mei 2020'></meta>
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <link rel="icon" type="image/x-icon" href="gambar/iconk.jpg" />
        <link rel="stylesheet"href="stylelist.css"></link>
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
        <link rel="stylesheet" href="https://www.w3schools.com/w3css/4/w3.css">
        <script src="script.js" type="text/javascript"></script>
    </head>
    <header> 
        <div class="logo"><img src="gambar/logos.png" alt="logo" width="100" height="100"></div>
         <nav>
             <ul>
                 <li><a href="index.html"id="back">Home</a></li>
                 <li><a href="about.html">About</a></li>
                 <li><a href="contact.html">Contact</a></li>
                 <li><a href="loginto.html">Login</a></li>	
             </ul>
         </nav> 
<body>
   <?php
   
   ?> 
    <center>
        <h4>Masuk Sebagai</h4>
        <div class="content">
            <a href="login.php">
            <div class="profiles1"><img src="gambar/Customer.png" alt="logo" width="200" height="200"></div>
            </a>
        <br></br>
        </div>
        <div class="content">
            <a href="login11.php">
            <div class="shoping1"><img src="gambar/staf.png" alt="logo" width="200" height="200"></div>
            </a>
        </div>
        <div class="content">
            <a href="login12.html">
            <div class="shoping2"><img src="gambar/Owner.png" alt="logo" width="200" height="200"></div>
            </a>
        </div>
        </div>
        <div class="content">
            
        </div>
        <div class="content">
            
        </div>
    </div>
    
</center>

<footer>
    <marquee><p>© Apotek Senja</p></marquee>
</footer>

</body>
</html>